...
I prefer editing the data import code because it's more simple than the other option. The other option adds more lines of code overall.
...
This plot is a result of setting the beside argument to F, where the different foods are stacked on top of each other.
...
Compared to other countries in the UK, people in N. Ireland eat more potatoes and less fresh fruit.
...
PC2 is mainly influenced by the soft drinks and fresh potatoes food groups.There are higher PC2 scores with soft drinks and lower PC2 scores with fresh potatoes. PC2 provides us with information from an additional 29% of the data that contribute to N. Ireland's position on the PCA plot. 
...
In this data set, there are 100 genes, # of rows, and 10 samples, # of columns.

```{r}
## Take the transpose of our data 
pca <- prcomp(t(rna.data), scale=TRUE)
 
## Unpolished plot of pc1 and pc2
...
```{r}
## Wt and ko samples - vector for colors
...
```{r}
# Add a column for 'wt' and 'ko' "condition" 
